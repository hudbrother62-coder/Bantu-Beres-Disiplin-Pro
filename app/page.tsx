...
export default function Home(){
 ...
 async function load(){if(!session?.user)return;let {data:s}=await sb.from('schools').select('*').eq('owner_user_id',session.user.id).maybeSingle();if(!s){const m=session.user.user_metadata||{};const created=await sb.from('schools').insert({owner_user_id:session.user.id,name:m.school_name||'Sekolah Baru',principal_name:m.principal_name||null,npsn:m.npsn||null}).select().single();s=created.data}setSchool(s||null);if(!s)return;const [a,b,c,d,e]=await Promise.all([sb.from('students').select('*').eq('school_id',s.id).order('name'),sb.from('master_items').select('*').eq('school_id',s.id).order('type').order('name'),sb.from('incidents').select('*,students(name,class_name,nis)').eq('school_id',s.id).order('occurred_at',{ascending:false}).limit(500),sb.from('coaching_sessions').select('*,students(name,class_name)').eq('school_id',s.id).order('happened_at',{ascending:false}).limit(300),sb.from('student_actions').select('*,students(name,class_name)').eq('school_id',s.id).order('created_at',{ascending:false}).limit(300)]);setStudents(a.data||[]);setMaster(b.data||[]);setIncidents(c.data||[]);setCoaching(d.data||[]);setActions(e.data||[])}
 async function auth(e:any){e.preventDefault();setMsg('');setBusy(true);const authEmail=credentialToEmail(email);try{if(authMode==='login'){const {error}=await sb.auth.signInWithPassword({email:authEmail,password});if(error)setMsg(friendlyAuthError(error.message));return}if(!schoolName.trim()){setMsg('Nama sekolah wajib diisi.');return}if(!principalName.trim()){setMsg('Nama kepala sekolah wajib diisi.');return}const metadata={school_name:schoolName.trim(),principal_name:principalName.trim(),npsn:npsn.trim(),login_label:email.trim()};const {data,error}=await sb.auth.signUp({email:authEmail,password,options:{data:metadata}});if(error){await sb.rpc('activate_recent_signup_by_email',{p_email:authEmail});const retry=await sb.auth.signInWithPassword({email:authEmail,password});if(!retry.error){setMsg('');return}setMsg(friendlyAuthError(error.message));return}if(data.user&&!data.session){await sb.rpc('activate_new_signup',{p_user_id:data.user.id,p_email:authEmail});const signed=await sb.auth.signInWithPassword({email:authEmail,password});if(signed.error){setMsg(friendlyAuthError(signed.error.message));return}}setMsg('')}finally{setBusy(false)}}
